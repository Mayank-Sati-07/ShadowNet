...
export default function PersonInvestigation() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8 pb-10">
      ...
      <motion.div variants={itemVariants} className="mt-8 rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] overflow-hidden shadow-sm">
        <div className="flex items-center gap-3 text-[var(--color-foreground)] border-b border-[var(--color-border)] p-4">
          <Network size={20} className="text-[var(--color-primary)]" />
          <h2 className="font-semibold text-[var(--color-foreground)]">Visual Network Graph</h2>
        </div>
        <NetworkGraph network={details} />
      </motion.div>
    </div>
  );
}