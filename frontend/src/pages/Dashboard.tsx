      ...
      >
        <div className="mb-4 flex items-center justify-between">
          ...
          <button
            ...
            className="bg-[var(--color-primary)]/10 border border-[var(--color-primary)]/30 px-4 py-2 text-sm font-medium text-[var(--color-primary)] hover:bg-[var(--color-primary)]/20 rounded-lg transition-all duration-300 shadow-sm hover:shadow-md"
          >
            Open Network Explorer
          </button>
        </div>

        <div className="overflow-hidden rounded-xl border border-[var(--color-border)]/50 bg-[var(--color-card)]/40 backdrop-blur-md shadow-sm">
          <table className="w-full text-left text-sm">
            <thead className="bg-[var(--color-muted)]/50 text-[var(--color-muted-foreground)] border-b border-[var(--color-border)]/50">
              <tr>
              ...
              {topPersons.length === 0 ? (
                ...
                topPersons.map((person) => (
                  <tr
                    ...
                    className="cursor-pointer transition-colors hover:bg-[var(--color-muted)]/50"
                  >
                    ...
                    <td className="p-4">
                       <span className="rounded-lg border border-[var(--color-secondary)]/30 bg-[var(--color-secondary)]/10 px-3 py-1 text-xs font-medium text-[var(--color-secondary)] shadow-sm">
...
function StatCard({ title, value }: { title: string; value?: number | string | null }) {
  return (
    <motion.div 
      ...
      className="rounded-xl border border-[var(--color-border)]/50 bg-[var(--color-card)]/40 backdrop-blur-xl p-6 hover:border-[var(--color-primary)]/40 transition-all duration-300 relative overflow-hidden group shadow-sm hover:shadow-md"
    >
      <div className="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-[var(--color-primary)]/5 blur-3xl group-hover:bg-[var(--color-primary)]/10 transition-colors duration-500" />
      <div className="flex items-center gap-2">
        ...
        <p className="text-sm font-medium text-[var(--color-muted-foreground)] uppercase tracking-wider">{title}</p>
      </div>
      <h2 className="mt-3 text-4xl font-bold tracking-tight text-[var(--color-foreground)] font-mono">{value ?? 0}</h2>
    </motion.div>
  );
}
